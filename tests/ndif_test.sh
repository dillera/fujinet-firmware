#!/usr/bin/env bash
#
# ndif_test.sh - byte-identical comparison of lib/stuffit/ndif.c's NDIF
# (Disk Copy 6) decoder against ndif2raw (~/code/FujiNet_macOS_2026/ref_ndif2raw/,
# the independent reference implementation ndif.c was ported from), for
# every "*.img" entry inside Now_Software.sit (a real-world StuffIt archive
# whose disk images are all NDIF).
#
# For each such entry, this:
#   1. runs `unsit -i Now_Software.sit "<entry>" out.raw` - unstuffs the
#      entry (StuffIt decompression) and decodes its NDIF image
#      (lib/stuffit/ndif.c) in one step, producing a raw disk image.
#   2. runs `unar -D -f -q -o <dir> Now_Software.sit` once (unar's default
#      "fork" mode attaches each entry's resource fork verbatim as the
#      com.apple.ResourceFork xattr on the extracted data-fork file - see
#      stuffit_test.sh's own comment for why this mode and not `-k visible`),
#      then feeds the matching extracted file straight to
#      `ndif2raw --format=resource-fork <file> oracle.raw` - ndif2raw reads
#      the resource fork (the 'bcem' block map lives there) via the
#      classic Mac Resource Manager API, which macOS still backs with the
#      com.apple.ResourceFork xattr unar just wrote.
#   3. `cmp`s out.raw against oracle.raw, byte for byte.
#   4. also runs ndif_blocks_test (a tiny standalone harness built
#      alongside unsit - see tests/CMakeLists.txt) against the same entry,
#      spot-checking ndif_read_blocks() (the random-access path
#      ndif_extract()/`unsit -i` never exercises) against oracle.raw.
#
# ndif2raw itself is dev-machine-only tooling (not vendored, not built as
# part of this project's normal build): `make -C $NDIF2RAW_DIR` is run
# once up front, and if that's unavailable or fails (no `make`, no Xcode
# command line tools, ...) this whole test is SKIPPED (exit 0, with a
# clear stderr message) rather than failed, the same "don't hard-fail CI
# over dev-only tooling" convention stuffit_test.sh's own missing-unar
# case follows at the CMakeLists.txt level - this one additionally has to
# apply it at run time, since `make` being *present* doesn't guarantee the
...
NDIF2RAW_DIR="${NDIF2RAW_DIR:-$HOME/code/FujiNet_macOS_2026/ref_ndif2raw}"
NOW_SOFTWARE_SIT="${NOW_SOFTWARE_SIT:-$HOME/code/FujiNet_macOS_2026/sit_samples/Now_Software.sit}"
