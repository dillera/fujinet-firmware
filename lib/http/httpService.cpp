...
#ifdef BUILD_MAC
// Streams the disk image a StuffIt/BinHex archive was unstuffed into for
// a given drive slot, straight out of the PSRAM buffer sitMount.cpp
// extracted it into (see lib/media/mac/sitMount.h). 404 if that slot
// isn't currently backed by an archive.
esp_err_t fnHttpService::get_handler_sitdownload(httpd_req_t *req)
{
    ...
    // Final zero-length chunk signals EOF to the chunked-encoding machinery.
    httpd_resp_send_chunk(req, nullptr, 0);
